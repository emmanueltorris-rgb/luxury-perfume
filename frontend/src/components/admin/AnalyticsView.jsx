      ...
      <div className="liquid-glass p-4 sm:p-6">
        ...
        <div className="space-y-2">
          {STATUS_OPTIONS.map((s) => {
            ...
            return (
              <div key={s} className="flex min-w-0 items-center gap-2 sm:gap-3">
                <span className="w-16 shrink-0 text-xs capitalize text-espresso/70 sm:w-24 sm:text-sm">{s}</span>
                <div className="flex-1 h-3 rounded-full bg-[#FDFBF7] overflow-hidden">
                  <div className="h-full bg-[#FF7F62]" style={{ width: `${pct}%` }} />
                </div>
                <span className="w-8 shrink-0 text-xs text-right text-espresso/70 sm:w-10 sm:text-sm">{count}</span>
              </div>
            )
          })}
        </div>
